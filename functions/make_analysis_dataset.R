make_analysis_dataset <- function(data, imputed=FALSE, data_to_merge=NULL) {
    ...
    if(max(abs(data[ , other_inc - fam_tax - fam_unearned_non_gov - fam_unearned_gov - oth_labern])) != 0) stop("Sum of components of other income do not equal family income.")
